...
export default function PortfolioSection() {
  ...
  const applyAllChanges = async () => {
    ...
    for (const { post, imageFile } of newPosts) {
      ...
      try {
        ...
        if (!createResult.success) {
          await portfolioActions({ type: 'ROLLBACK_CREATE', postId: 0, imagePath }).catch(() => {});
          errors.push(`"${post.title_en}": ${createResult.error || 'failed to create post'}`);
        }
      } catch (e) {
        if (uploadedImagePath) {
          await portfolioActions({ type: 'ROLLBACK_CREATE', postId: 0, imagePath: uploadedImagePath }).catch(() => {});
        }
    ...
    await fetchPortfolioData().catch(() => {});
    setModifiedPosts(new Set());
    setNewPosts([]);
    setDeletedPosts(new Set());
    setIsUpdating(false);
