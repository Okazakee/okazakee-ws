...
export default async function Skills() {
  ...
  return (
    skills_categories ?
      <section id="skills" className="flex items-center justify-center text-center mx-5 xl:mx-16 md:min-h-lvh mt-20 md:mt-0 mb-20 md:mb-32 mdh:mt-40">
        <div className="w-full h-full ">
          <h1 className="xl:text-6xl text-3xl xs:text-4xl mb-10 xl:mb-5 ">{t('title')}</h1>
          <h2 className="xl:mb-20 text-lg xs:text-[1.4rem] md:text-2xl mb-10"
            dangerouslySetInnerHTML={{ __html: formatLabels(t('subtitle')) }}
          ></h2>
          {skills_categories.map((skillCategory, index) => (
            <div key={index} className="">
              <h2 className="text-[1.4rem] xs:text-[1.7rem] md:text-[2.33rem] xs:tracking-wider my-5">{t("skills." + skillCategory.name)}</h2>
              <div className="flex xl:flex-nowrap flex-wrap justify-center items-center">
                {skillCategory.skills.map((skill, i) => (
                  <div key={i} className="md:drop-shadow-3xl drop-shadow-xl dark:drop-shadow-none hover:scale-110 transition-all mx-auto my-5 md:my-10 w-[calc(33.333%-1rem)]">
                    <Image
                      placeholder='blur'
