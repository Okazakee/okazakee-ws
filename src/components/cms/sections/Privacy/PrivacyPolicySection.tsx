...
export default function PrivacyPolicySection() {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch { setError(t('privacy.errorFetch')); }
  ...
  const handlePublish = useCallback(async () => {
    ...
    for (const locale of ['en', 'it'] as const) {
      const r = await i18nActions({
        ...
        sectionData: { privacy_policy: locale === 'en' ? enMarkdown : itMarkdown },
      } as Parameters<typeof i18nActions>[0]);
  ...
  const textareaClass = 'w-full px-4 py-3 bg-white dark:bg-darkestgray border border-gray-300 dark:border-lighttext2/30 rounded-lg text-darktext dark:text-lighttext focus:border-main focus:outline-none font-mono text-sm resize-y';
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <SectionHeader title={t('privacy.title')} description={t('privacy.subtitle')} />
      ...
      <div>
        <h2 className="text-lg font-bold text-main mb-3">{activeLocale === 'en' ? t('common.english') : t('common.italian')}</h2>
        <textarea
          ...
          placeholder={activeLocale === 'en' ? '# Privacy Policy' : '# Informativa sulla Privacy'}
        />
      </div>
      ...
      {showPreview && (
        <div className="bg-gray-100 dark:bg-darkergray rounded-xl p-4 md:p-6">
          ...
          <div className="prose dark:prose-invert max-w-none bg-white dark:bg-darkestgray rounded-lg p-4 md:p-6 border border-gray-200 dark:border-darkgray/50">
            <MarkdownRenderer markdown={activeLocale === 'en' ? enMarkdown : itMarkdown} />
          </div>
        </div>
      )}
