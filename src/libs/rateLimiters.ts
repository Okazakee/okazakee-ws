...
export function checkLoginRateLimit(identifier: string): {
  ...
  if (entry.count >= LOGIN_MAX_ATTEMPTS) {
    ...
    if (timeSinceLastAttempt < LOGIN_LOCKOUT_DURATION) {
      const lockoutRemaining = Math.ceil((LOGIN_LOCKOUT_DURATION - timeSinceLastAttempt) / 1000);
  ...
  if (now - entry.timestamp < LOGIN_RATE_LIMIT_DURATION) {
    ...
    if (entry.count >= LOGIN_MAX_ATTEMPTS) {
      return { allowed: false, remainingAttempts: 0, lockoutRemaining: LOGIN_LOCKOUT_DURATION / 1000 };
    }
    
    return { allowed: true, remainingAttempts: LOGIN_MAX_ATTEMPTS - entry.count };
  }
